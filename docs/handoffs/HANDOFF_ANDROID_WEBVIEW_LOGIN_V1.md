...
Implementation branch: `feat/android-webview-login-v1`

Implementation base commit: `74b801dea2dff2fd360aefe596cc37ba3e4eb4db`

Current auth baseline:
- PR #2 has been squash-merged.
- Main baseline: `74b801dea2dff2fd360aefe596cc37ba3e4eb4db`.
- The merged implementation was reviewed at head `7c40c0ec7898c8af21cc24b6120cd501c47a54e5`.
- Manual Android CI Run #42 passed unit tests, lint, assembleDebug, and artifact upload on that reviewed head.
- Owner-device baseline remains mixed: some Instagram / Threads / X samples work and some still fail. WebView Login V1 is intended to make validated authenticated fallback usable on-device; it must not turn this milestone into another broad anonymous extractor rewrite.
- Implementation starts from the merged main baseline above.
...
PR #2 is merged. WebView Login V1 now uses:
...
Implement only this handoff on the feature branch and open a new PR after Phase 1 has passed local gates.

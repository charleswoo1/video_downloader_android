...
object MetaFfmpegHelper {
    ...
    fun resolveRuntimePaths(nativeLibraryDir: File, noBackupFilesDir: File): FfmpegRuntimePaths {
        val executable = File(nativeLibraryDir, "libffmpeg.so").takeIf { it.exists() }
            ?: File(noBackupFilesDir, "youtubedl-android/packages/ffmpeg/ffmpeg").takeIf { it.exists() }
            ?: File(noBackupFilesDir, "youtubedl-android/packages/ffmpeg/bin/ffmpeg").takeIf { it.exists() }
